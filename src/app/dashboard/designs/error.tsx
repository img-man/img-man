...
/**
 * Design Studio error boundary.
 * Catches errors in the /dashboard/designs routes (editor crashes, etc.)
 */
export default function DesignsError({
  ...
  useEffect(() => {
    console.error('[Designs Error Boundary]', error);
  }, [error]);
    ...
    <div className="flex min-h-[60vh] items-center justify-center p-8">
      <div className="flex max-w-md flex-col items-center gap-4 rounded-2xl border border-red-200 bg-red-50 px-8 py-10 text-center dark:border-red-900/50 dark:bg-red-950/30">
        ...
        <p className="text-sm text-red-600 dark:text-red-400">
          {error.message || 'The design editor encountered an error.'}
        </p>
        <button
          onClick={reset}
