...
/**
 * PDF Split Tool
 * Upload a PDF and split it by page ranges or into individual pages.
 * Entirely client-side via pdf-lib.
 */

import { useState, useCallback, useRef } from 'react';
import { X, Upload, Download, Loader2, Scissors, Library } from 'lucide-react';
...
export default function PdfSplitModal({ onClose }: { onClose: () => void }) {
  ...
  /** Parse ranges like "1-3, 5, 7-9" into an array of page indices (0-based) */
  ...
  const handleSplit = useCallback(async () => {
    ...
    const indices = parseRanges(state.rangeInput);
    if (!indices) {
      setState((s) => ({
        ...s,
        error: `Invalid range. Use 1-${s.totalPages} format.`,
      }));
      return;
    }
    ...
    try {
      ...
      setState((s) => ({
        ...s,
        resultBlob: blob,
        resultName: `${baseName}_split.pdf`,
      }));
    } catch (err) {
      setState((s) => ({
        ...s,
        error: `Split failed: ${err instanceof Error ? err.message : 'Unknown error'}`,
      }));
    } finally {
      setState((s) => ({ ...s, processing: false }));
    }
  }, [state.file, state.fileName, state.rangeInput, parseRanges]);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-dash-border px-6 py-4">
          <div>
            ...
            <p className="text-xs text-dash-text-muted mt-0.5">
              Extract pages or split a PDF by page ranges
            </p>
          </div>
          <button
            onClick={onClose}
        ...
        <div className="p-6 space-y-4 max-h-[70vh] overflow-y-auto">
          ...
          {!state.file ? (
            ...
            <>
              ...
              <div className="flex items-center gap-3 rounded-lg border border-dash-border bg-dash-muted/50 px-4 py-3">
                <Scissors className="h-5 w-5 text-orange-500 shrink-0" />
          ...
          {/* Error */}
          {state.error && (
            <p className="text-xs text-red-500" data-testid="pdf-split-error">
              {state.error}
            </p>
          )}
